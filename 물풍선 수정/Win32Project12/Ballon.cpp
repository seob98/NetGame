	...
	: pos(Pos), size(Size), index(Number), explosionLength(_explosionLength), summonerClientNum(_summonerClientNum), summoner(_summoner)/*, player0(_player0)*/
{
	...
	CBmpMgr::Get_Instance()->Insert_Bmp(L"CrazyArcadeImage/Image/Bullet/bullet.bmp", L"ballon");	// 이미지를 어디서 넣든 상관없습니다. 중복은 예외처리합니다.
...
{
	HDC hMemDC = CBmpMgr::Get_Instance()->Find_Image(L"ballon");
...
void CBallon::CheckPlayerOut(std::vector<CPlayer>& players)
{
 	...
 	for (int i = 0; i < MAX_PLAYER_CNT; ++i)
	{
		...
		if (!IntersectRect(&temp, &rt, &players[listOfIntersected[i]].GetRect()))
...
void CBallon::CheckCollision(std::vector<CPlayer>& players)
{
	...
	for (int i = 0; i < MAX_PLAYER_CNT; ++i)
	{
		...
		{
			if (IntersectRect(&temp, &players[i].GetRect(), &rt))
			{
